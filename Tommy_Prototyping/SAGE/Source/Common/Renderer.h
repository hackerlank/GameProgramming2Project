...
//16 vs 32-bit index buffers
//For a crappy ond computer, comment out the following define
//and stick to a 256x256 height map.
//If you have a decent graphics card, uncomment the following define
//and then you can have a larger height map.

#define INDEX_BUFFER_32 //define for 32-bit index buffers

#ifdef INDEX_BUFFER_32 //32-bit index buffers
  #define SAGE_D3DFMT_INDEX D3DFMT_INDEX32
  #define INDEX_BUFFER_INDEXTYPE int
#else //16-bit index buffers
  #define SAGE_D3DFMT_INDEX D3DFMT_INDEX16
  #define INDEX_BUFFER_INDEXTYPE short
#endif
...
struct RenderTri
{
#ifdef INDEX_BUFFER_32
  unsigned int index[3]; ///< Array of vertex indices forming the triangle
#else
...
public:
  ...
  void render(VertexBufferBase *vb, int vertStart, int vertCount, IndexBuffer *ib, int triStart, int triCount, TextureReference& t);
...
protected:
	...
	void initializeVertexDeclarations();
	void createEffect();
	void loadEffectHandles();

	IDirect3DVertexDeclaration9* m_modelVertexDecl;

	ID3DXEffect*	m_effect;
	std::string		m_effectFilePath;
	D3DXHANDLE		m_effectHandle;
	D3DXHANDLE		m_textureHandle;
	D3DXHANDLE		m_wvpMatrix;
