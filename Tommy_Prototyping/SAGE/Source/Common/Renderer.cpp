...
Renderer::Renderer()
: m_modelVertexDecl(nullptr),
  m_effect(nullptr),
  m_effectFilePath("Model.fx")
{
...
void Renderer::render(VertexBufferBase *vb, int vertStart, int vertCount, IndexBuffer *ib, int triStart, int triCount, TextureReference& t)
{
  // record number of triangles that are rendered
  nTriangleFrameCount += triCount;
  ...
  // tell DX our desired vertex format
  hres = pD3DDevice->SetVertexDeclaration(m_modelVertexDecl);

  hres = m_effect->SetTechnique(m_effectHandle);

  hres = m_effect->SetTexture(m_textureHandle, textureCacheList[t.handle]->d3dTexture);
  hres = m_effect->SetMatrix(m_wvpMatrix, &(m_world * m_view * m_projection));

  // draw our geometry
  unsigned number_of_passes(0);
  m_effect->Begin(&number_of_passes, 0);
  for(unsigned current_pass = 0; current_pass < number_of_passes; ++current_pass)
  {
	  m_effect->BeginPass(current_pass);
	  hres = pD3DDevice->DrawIndexedPrimitive(
		D3DPT_TRIANGLELIST,
		vertStart,
		0,
...
void Renderer::initializeVertexDeclarations()
{
	if(m_modelVertexDecl)
		return;
	...
	HRESULT vertDeclCreation = pD3DDevice->CreateVertexDeclaration(vertex_position, &m_modelVertexDecl);
	assert(SUCCEEDED(vertDeclCreation));
}

void Renderer::createEffect()
{
	...
	std::string dir = gDirectoryManager.getDirectory();

	HRESULT effectCreation = D3DXCreateEffectFromFile(
		pD3DDevice,
		m_effectFilePath.c_str(),
		nullptr,
		nullptr,
		shader_flags,
		nullptr,
	...
	if(FAILED(effectCreation))
		DXTrace(__FILE__, (DWORD)__LINE__, effectCreation, "Effect Creation Failed", false);

	if(error_buffer)
	{
		OutputDebugString(reinterpret_cast<char*>(error_buffer->GetBufferPointer()));
		error_buffer->Release();
	}
}

void Renderer::loadEffectHandles()
{
	m_effectHandle	= m_effect->GetTechniqueByName("ModelRender");
	m_textureHandle = m_effect->GetParameterByName(0, "g_texture");
	m_wvpMatrix		= m_effect->GetParameterByName(0, "g_world_view_projection");
	m_ambientLight	= m_effect->GetParameterByName(0, "g_ambient_light");
}
