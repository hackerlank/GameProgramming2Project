/// \file EditTriMesh.cpp
/// \brief Code for the EditTriMesh class.
...
/// \param vertexIndex Index (mesh based) of the vertex to locate
/// \return Index of face vertex (0..2), or -1 if we don't use that vertex
...
/// \param degrees The tolerance in degrees
...
/// \return The specified vertex data
...
/// \param triIndex Index of the triangle
/// \return The specified triangle data
...
/// \return The specified material data
...
/// \return The specified part data
...
/// \param newCount Desired size of the list
/// \note If the lists are grown, the new entries will be properly defaulted.
/// If the lists are shrunk, no check is made to ensure that a valid mesh
/// remains.
void	EditTriMesh::setVertexCount(int newCount) {
	assert(newCount >= 0);
	...
	if (newCount > vCount) {
		...
		while (vCount < newCount) {
			vList[vCount].setDefaults();
			++vCount;
		}

	} else if (newCount < vCount) {
		...
		for (int i = 0 ; i < triCount() ; ++i) {
			Tri *t = &tri(i);
			if (
				(t->v[0].index >= newCount) ||
				(t->v[1].index >= newCount) ||
				(t->v[2].index >= newCount)
			) {
...
/// If the lists are shrunk, no check is made to ensure that a valid mesh
/// remains.
void	EditTriMesh::setTriCount(int newCount) {
	assert(newCount >= 0);
...
/// If the lists are shrunk, no check is made to ensure that a valid mesh
/// remains.
void	EditTriMesh::setMaterialCount(int newCount) {
	assert(newCount >= 0);
	...
	if (newCount > mCount) {
		...
		mList = (Material *)::realloc(mList, newCount * sizeof(*mList));
...
/// If the lists are shrunk, no check is made to ensure that a valid mesh
/// remains.
void	EditTriMesh::setPartCount(int newCount) {
	assert(newCount >= 0);
	...
	if (newCount > pCount) {
		...
		pList = (Part *)::realloc(pList, newCount * sizeof(*pList));
		...
		while (pCount < newCount) {
			pList[pCount].setDefaults();
			++pCount;
		}

	} else if (newCount < pCount) {
...
/// \param srcVertexIndex Source vertex index to duplicate
/// \return Index of the newly added item
...
/// \param mark Value to assign to all vertex marks
...
/// \param mark Value to assign to all triangle marks
...
/// \param mark Value to assign to all material marks
...
/// \param mark Value to assign to all part marks
...
/// \param vertexIndex Index of the vertex to delete
...
/// \param triIndex Index of the triangle to delete
...
/// \param materialIndex Index of the material to delete
...
/// \param partIndex Index of the part to delete
...
void	EditTriMesh::deleteUnusedMaterials() {
	...
	// and where they will go into the new material list,
...
void	EditTriMesh::deleteEmptyParts() {
	...
	// and where they will go into the new part list,
	// after the unused ones are removed
...
/// This creates a new vertex list, with each vertex only used by one
/// triangle. Simultaneously, unused vertices are removed.
...
/// \param m Matrix by which to transform all the vertices in the mesh
...
/// \param meshes Destination array of meshes
/// \note The caller must allocate the memory for the meshes, and the number
/// of meshes needs to equal the number of parts
/// \code
/// EditTriMesh *meshes = new EditTriMesh[mesh.partCount()];
/// mesh.extractParts(meshes);
/// \endcode
...
/// \param partIndex Index of the desired part to extract
/// \param materialIndex Index of the desired material to extract
/// \param result Pointer to the resulting EditTriMesh
/// \note The caller must allocate the memory for the mesh
/// \code
/// EditTriMesh *newMesh = new EditTriMesh();
/// oldMesh.extractOnePartOneMaterial(pi, mi, newMesh);
/// \endcode
...
void	EditTriMesh::computeOneTriNormal(Tri &t) {
	...
	t.normal = Vector3::crossProduct(e1, e2);
...
/// \remark This automatically computes the triangle level surface normals
...
/// \return The axially-aligned bounding box of the mesh
...
/// \remark This can improve cache performace and vertex caching by
/// increasing the locality of reference. This function can also remove
/// unused vertices simultaneously
/// \param removeUnusedVertices Set to true to remove unused vertices, false
...
/// \remark This is VERY important for effecient rendering
...
/// \param opt Set of optimatization parameters to use when welding
...
/// \remark Prepares the model for fast rendering under *most* rendering
/// systems, with proper lighting.
...
/// \param returnErrMsg Buffer for the return error message
/// \param defaultDirectory Whether or not to set the load directory to the
/// model default
/// \return True if the import was successful, false otherwise
bool	EditTriMesh::importS3d(const char *filename, char *returnErrMsg,size_t errMsgSize, bool defaultDirectory) {
	int	i;

	// set directory to model directory if requested
	if (defaultDirectory)	
...
corrupt:
		...
		strcpy_s(returnErrMsg,errMsgSize, "File is corrupt");
		goto failed;
	}
	int	version;
	//SECURITY-UPDATE:2/3/07
	...
	if (version != 103) {
		...
		sprintf_s(returnErrMsg,errMsgSize, "File is version %d - only version 103 supported", version);
		goto failed;
	}
	...
	for (i = 0 ; i < numParts ; ++i) {
		...
		if (fscanf_s(f, "%d , %d , %d , %d , \"%[^\"]\"\n", &partFirstVert, &partNumVerts, &partFirstTri, &partNumTris, p->name,sizeof(p->name)) != 5) {
			//SECURITY-UPDATE:2/3/07
			//sprintf(returnErrMsg, "Corrupt at part %d", i);
			sprintf_s(returnErrMsg,errMsgSize, "Corrupt at part %d", i);
			goto failed;
		}
		if (firstVert != partFirstVert || firstTri != partFirstTri) {
			//SECURITY-UPDATE:2/3/07
			//sprintf(returnErrMsg, "Part vertex/tri mismatch detected at part %d", i);
			sprintf_s(returnErrMsg,errMsgSize, "Part vertex/tri mismatch detected at part %d", i);
			goto failed;
		}
		p->mark = partNumTris;
	...
	if (firstVert != numVerts || firstTri != numTris) {
		...
		strcpy_s(returnErrMsg,errMsgSize, "Part vertex/tri mismatch detected at end of part list");
		goto failed;
	}
	...
	for (i = 0 ; i < numTextures ; ++i) {
		...
		if (fgets(m->diffuseTextureName, sizeof(m->diffuseTextureName), f) != m->diffuseTextureName) {
			...
			sprintf_s(returnErrMsg,errMsgSize, "Corrupt reading texture %d", i);
			goto failed;
		}

		// Strip off newline, which fgets leaves.
	...
	for (int partIndex = 0 ; partIndex < numParts ; ++partIndex) {
		...
		for (int i = 0 ; i < part(partIndex).mark ; ++i) {
			...
			if (fscanf_s(f, "%d , %d , %f , %f , %d , %f , %f , %d , %f , %f\n",
				&t->material,
				...
				sprintf_s(returnErrMsg,errMsgSize, "Corrupt reading triangle %d (%d of part %d)", destTriIndex, i, partIndex);
				goto failed;
			}
			...
			if (t->material < 0) {
				if (whiteTextureIndex < 0) {
					...
					strcpy_s(whiteMaterial.diffuseTextureName,sizeof(whiteMaterial.diffuseTextureName), "White");
	...
	for (i = 0 ; i < numVerts ; ++i) {
		...
		if (fscanf_s(f, "%f , %f , %f\n", &v->p.x, &v->p.y, &v->p.z) != 3) {
			...
			sprintf_s(returnErrMsg,errMsgSize, "Corrupt reading vertex %d", i);
			goto failed;
		}
	}
...
/// \param returnErrMsg Buffer to store the return error message
/// \todo Implement this function
